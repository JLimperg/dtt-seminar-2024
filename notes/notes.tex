...
Perhaps surprisingly, there are also practical implementations of extensional type theories, such as F* (TODO ref).
...
We now consider a particularly prominent type theory: intensional Martin-Löf type theory (MLTT), which lies at the heart of the Agda theorem prover (TODO ref) and of the axiomatic formulation of homotopy type theory (TODO ref).
Since Per Martin-Löf developed multiple type theories, the term MLTT is not exactly well-defined; we choose the theory published in 1975 (TODO ref) with an infinite hierarchy of universes.
The presentation is inspired by (but substantially different from) that on the nLab (TODO ref).
...
However, in this case, the universe cannot be a member of itself, so we cannot have $\Univ : \Univ$, as the type theory would then become inconsistent per Girard's paradox (TODO ref).
...
The type theory of Coq (TODO ref) includes a corresponding rule.
However, cumulativity seems to have a fairly minor impact on the convenience of working with a type theory in practice, so most other systems eschew it in favour of a simpler theory.

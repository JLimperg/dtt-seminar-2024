...
\begin{definition}[Full set-theoretic model of λL]
  ...
  We henceforth omit the subscripts of the interpretation functions since it will always be clear from context.
...
It may be instructive to check that the definition of the term interpretation function is \enquote*{type-correct}, i.e.\ that each case defines a function with the right domain and codomain.

The model we have defined serves two purposes.
First, it formalises our intuition about the various constructions of $λL$: abstractions can be interpreted as functions; the product type can be interpreted as the cartesian product; and so on.
This validates the design of λL.

Second, the model gives an easy proof of consistency:
\begin{proof}[Alternative proof of Theorem~\ref{th:consistency}]
  ...
  Such a function can only exist if $⟦∅⟧$ is the empty set, but $⟦∅⟧$ is the set of functions with empty domain, which has one trivial inhabitant.
